...
import {  filteredByPrice, activeCategoryChanged, filteredFavorite } from '../../actions/resultActions';
...
const Results = () => {
  ...
  const renderItems = (items) => {
    ...
    return items.map(el => {
      // const id = uuidv4();
      return (
        <li className="results__item product" key={el.id} >
            ...
            <form className="sorting__form">
              ...
              <div className="sorting__favourites">
                <input 
                  ...
                  type="checkbox" name="favourites" 
                  id="favourites" 
                  onClick={(e) => {
                    dispatch(filteredFavorite(e.currentTarget.checked))
                  }}/>
